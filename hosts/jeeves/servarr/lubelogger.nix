...
in
{
  services = {
    ...
    lubelogger.package = pkgs.callPackage (
      # temp fix until nixpkgs builds are fixed
      # The issue is with `deps.json` not reflecting updates since v1.4.5
      {
        lib,
        buildDotnetModule,
        dotnetCorePackages,
      ...
      buildDotnetModule rec {
        ...
        src = fetchFromGitHub {
          ...
          repo = "lubelog";
          rev = "v${version}";
          hash = "sha256-NkbKgIegNNITHYqOAwhTK3efUNAFOpiRcKJnN6Z1SC4=";
        };
